...
def convert_to_umkanzanion_song(a_string):
    ...
    return umkansanizanion_song, len(umkansanizanion_song_draft_list)
    

def make_and_write_the_songs(paths_with_song_names, names_with_translation, source_root, target_root):
    for keys, values in paths_with_song_names.items():
        ...
        if len(directory) == 0:
            with open(f"{target_root}/{keys}.txt", "w", encoding="utf-8") as song_files:
                print(names_with_translation[keys], file=song_files)
        else:
...
def Umkansanize(source_root:str, target_root:str) -> dict[str,int]:
    ...
    names_with_translation= {}
    tarahumara_to_umkansanion = {"0": "A", "1": "B", "2": "C", "3": "D", "4": "E", "5": "F", "6": "G", "-": "b", "+": "#", " ": "P"}
    for song_name, path in paths_with_song_names.items():    
        with open(os.path.join(source_root, path), "r", encoding="utf-8") as source_file:
            tarahumara_song = "".join(line.strip('\n')[::-1] for line in source_file)

            umkansanizanion_song_draft = "".join(map(lambda char: tarahumara_to_umkansanion[char], tarahumara_song))
            
            umkansanizanion_song, duration = convert_to_umkanzanion_song(umkansanizanion_song_draft)
            names_with_durations[song_name] = duration
            names_with_translation[song_name] = umkansanizanion_song

    
    sorted_songs = sorted(names_with_durations.items(), key=lambda item: (-item[1], item[0]))

    
    make_the_index_file(target_root, sorted_songs)
    make_and_write_the_songs(paths_with_song_names, names_with_translation, source_root, target_root)
...
if __name__ == "__main__":
    ...
    print(Umkansanize("test10", "test00001.out"))
    pass
